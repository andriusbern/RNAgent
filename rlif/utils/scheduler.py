...
A script for scheduling the training of multiple models
    ...
    params:
        * -e Environment_name
        * -p parameter_folder -> a folder containing *.yml files for the specific environment in ..rlif/config/{parameter_folder}
...
def run_experiment(env_name, parameter_folder , result_folder, verbose=False):
    ...
    for i, param_file in enumerate(os.listdir(parameter_dir)):
        ...
        model = Trainer(env_name, subdir=result_folder).create_model(config_location=parameters)
        sep = '\n' +'='*50 + '\n'
